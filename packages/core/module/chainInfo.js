...
export function createKeplrChainInfo(_ref) {
  let {
    currencies: _currencies
  } = _ref,
      chainInfo = _objectWithoutProperties(_ref, _excluded);
  ...
  for (const _currency of _currencies) {
    const currency = _objectSpread(_objectSpread({}, _currency), _currency.coinImageUrl && {
...
export const ChainInfoMap = Object.entries(SimpleChainInfoList).reduce((curr, [id, simplifiedChainInfo]) => _objectSpread(_objectSpread({}, curr), {}, {
  [id]: createKeplrChainInfo(simplifiedChainInfo)
}), {});
export const getChainInfo = async (chainId, chainInfoOverrides) => {
  const overrides = typeof chainInfoOverrides === 'function' ? await chainInfoOverrides() : chainInfoOverrides;
  const chainInfo = // Check overrides for chain info.
  overrides?.find(info => info.chainId === chainId) || // Use embedded map as fallback.
  ChainInfoMap[chainId];

  if (!chainInfo) {
    const availableChainIds = [...(overrides?.map(info => info.chainId) ?? []), ...Object.keys(ChainInfoMap).filter( // Don't list ID in overrides to prevent duplicates.
