...
import { addStateObservers, config, nextWalletConnectConnectionAttempt, onQrCloseCallback, setConfig, setOnQrCloseCallback, setWalletConnect, state, updateState, walletConnect, walletConnectConnectionAttempt } from './state';
...
export const initialize = (initialConfig, observers) => {
  // Setup internal state.
  setConfig(_objectSpread(_objectSpread({}, initialConfig), {}, {
  ...
  const maybeAutoConnect = async () => {
    ...
    const autoconnectWalletIndex = (await Promise.all(config.enabledWallets.map(({
      shouldAutoconnect
    }) => shouldAutoconnect?.() ?? false))).findIndex(value => value); // Autoconnect to wallet if exists.
    ...
    const localStorageValue = config.localStorageKey ? localStorage.getItem(config.localStorageKey) : null;
    const localStorageWallet = localStorageValue && config.enabledWallets.find(({
      id
    }) => id === localStorageValue); // If wallet found from localStorage value, auto connect.
...
export const connectToWallet = async wallet => {
  updateState({
    status: CosmosWalletStatus.Connecting,
    error: undefined,
    connectingWallet: wallet
  });
  let walletClient; // The actual meat of enabling and getting the wallet clients.

  const finalizeWalletConnection = async newWcSession => {
    updateState({
      status: CosmosWalletStatus.EnablingWallet
    });
    const chainInfo = await getChainInfo(config.defaultChainId, config.chainInfoOverrides);
    walletClient = await wallet.getClient(chainInfo, walletConnect, newWcSession);
    ...
    const connectedWallet = await getConnectedWalletInfo(wallet, walletClient, chainInfo, await config.getSigningCosmWasmClientOptions?.(chainInfo), await config.getSigningStargateClientOptions?.(chainInfo)); // Add refresh listener to update connected wallet info.

    wallet.addRefreshListener?.(refreshListener); // Allow to fail silently.

    try {
      await wallet.cleanupClient?.(walletClient); // Save localStorage value for future autoconnection.
  ...
  try {
    ...
    if (wallet.isWalletConnect) {
      ...
      if (!walletConnect) {
        setWalletConnect(new (await import('@walletconnect/client')).default({
        ...
        walletConnect._clientMeta = config.walletConnectClientMeta; // Detect disconnected WalletConnect session and clear wallet state.
      ...
      if (walletConnect.connected) {
        ...
        const currConnectionAttempt = nextWalletConnectConnectionAttempt(); // Executes walletConnect's qrcodeModal.open.

        await walletConnect.connect(); // If another connection attempt is being made, don't try to
...
export const beginConnection = async wallet => {
  ...
  const preselectedWallet = // If only one wallet is available, preselect it, no need to show modal.
  config.enabledWallets.length === 1 ? config.enabledWallets[0] : // Connect to preselected wallet if present.
  config.preselectedWalletId ? config.enabledWallets.find(({
    id
  }) => id === config.preselectedWalletId) : undefined;
...
export const disconnect = async dontKillWalletConnect => {
...
export const reset = async () => {
  // eslint-disable-next-line no-console
  await disconnect().catch(console.error); // Try resetting all wallet state and reconnecting.
